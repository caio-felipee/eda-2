...
int bfs(Graph *graph, int *dist, int s, int e) {
    ...
    while(!is_empty(queue)) {
        ...
        while(tmp.next != NULL) {
            ...
            if(dist[tmp.value] == MAX) {
                dist[tmp.value] = dist[f] + 1;
                insert(queue, tmp.value);

                if(tmp.value == e)
                    return dist[tmp.value];
            }
        }
    }
...
int main() {
    ...
    printf("%d\n", bfs(graph, dist, search(ht, fst)->value, search(ht, snd)->value));
}